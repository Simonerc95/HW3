...
path = 'C:\\Users\\simon\\Desktop\\ADM\\Homework 3'
...
reverse_voc = reverse_voc = {v:k for k,v in vocabulary.items()} #used to translate terms to IDs

with open(path + "\\WORDS\\DocWords.json", encoding = "utf-8") as fd:
        docwords = json.load(fd)

with open(path + "\\WORDS\\TfIdf_inv_index.json", encoding = 'utf-8') as tf:
        inv_ind_tfIDF= json.load(tf)
